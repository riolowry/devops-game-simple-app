...
README.md                 this file
SUPABASE_SETUP.md         create the backend (5 minutes)
DEPLOYMENT.md             deploy the frontend to Cloudflare Pages
...
1. **Create the backend.** Follow [SUPABASE_SETUP.md](SUPABASE_SETUP.md). Takes about 5 minutes. You get a `SUPABASE_URL` and `SUPABASE_ANON_KEY`.

2. **Configure the frontend.** Copy `config.example.js` to `config.js` and paste in the URL and key.

3. **Deploy.** Follow [DEPLOYMENT.md](DEPLOYMENT.md) to upload to Cloudflare Pages. Free, no Git required.
...
- Pylayeva, D. (2024). *DevSecOps Adventures: A Game-Changing Approach with Chocolate, LEGO, and Coaching Games*. Apress. [doi.org/10.1007/979-8-8688-0397-0](https://doi.org/10.1007/979-8-8688-0397-0)
